...
monthlyExpenses.forEach(exp => {
  ...
  return ( 
  <div className="bg-slate-900 min-h-screen pt-5 text-slate-100 flex flex-col gap-5">
    <Navbar />
    ...
    <div className="flex flex-col lg:flex-row gap-5 mx-[2%]">
      ...
      <div className="bg-slate-800 rounded-2xl p-4 lg:w-1/2">
        <h1 className='text-slate-400 font-extrabold text-xl flex justify-center mb-5'>Months</h1>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {months.map((month, index) => (
            <button
              key={month}
              onClick={() => setSelectMonth(index)}
              className={`py-2 px-2 cursor-pointer font-semibold rounded active:scale-95 text-sm
                ${selectMonth === index 
      ...
      <div className="bg-slate-800 rounded-2xl flex flex-col lg:w-1/2">
        <h1 className="font-extrabold text-xl sm:text-2xl text-slate-400 text-center mt-4">
          ...
          {monthlyExpenses.length === 0 ? (
            ...
            <ul className="space-y-2">
              {monthlyExpenses.map((exp) => (
                <li
                  key={exp.id}
                  className="flex justify-between items-center px-4 py-2 rounded text-slate-100 bg-slate-500 font-semibold"
                >
        ...
        <div className="py-3 flex justify-center font-bold">
          <span className="border border-slate-400 px-3 py-1 rounded bg-slate-500">
            TOTAL: ₹{totatAmount}
          </span>
        </div>
      </div>
    ...
    <div className="flex flex-col lg:flex-row gap-5 mx-[2%]">
      ...
      <div className="bg-slate-800 rounded-xl p-4 lg:w-1/2 flex items-center justify-center">
        <div className="w-full">
